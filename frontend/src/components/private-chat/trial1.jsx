...
const PrivateChat = () => {
  ...
  useEffect(() => {
    const fetchChatrooms = async () => {
      ...
      try {
        console.log(`/api/chatrooms/search/${userId}`);
        const res = await fetch(`http://localhost:5000/api/chatrooms/search/${userId}`);
        const data = await res.json();

        // Map backend data to the same shape as your existing mock data
        const formatted = data.map((room) => ({
          ...
          timestamp: room.lastMessageTime
            ? new Date(room.lastMessageTime).toLocaleTimeString("en-US", {
              hour: "numeric",
              minute: "2-digit",
              hour12: true,
            })
                    ...
                    <div className="flex items-center">
                      ...
                      {conversation.isTyping && (
                        <div className="ml-2 flex space-x-1">
                          ...
                          <div className="w-1 h-1 bg-purple-500 rounded-full animate-bounce delay-400" />
                        </div>
                      )}
                    </div>
                  </div>
